...
class SearchPatientViewModel: ObservableObject {
    ...
    private var unfilteredPatients = [Patient]()
    ...
    func fetchPatientsData() {
        db.collection("patients")
            ...
            self.unfilteredPatients = documents.map { (queryDocumentSnapshot) -> Patient in
        ...
        searched = true
        patients = unfilteredPatients.filter { $0.name.contains(searchText) || String($0.cic).contains(searchText) || $0.surname1.contains(searchText)
        }
    }
}
